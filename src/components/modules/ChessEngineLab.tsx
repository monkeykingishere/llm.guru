    ...
    <div className="w-full">
      ...
      <AnimatePresence mode="wait">
        {activeTab === "lab" ? (
          ...
          >
            {/* ---------------------------------------------------- */}
            {/* ROW 1 LEFT: CHESSBOARD CARD */}
            {/* ---------------------------------------------------- */}
            <div className="glass-strong rounded-3xl p-6 border border-white/10 h-[500px] flex flex-col justify-between">
              <div className="flex items-center justify-between mb-4 shrink-0">
                <div className="text-[11px] font-mono text-muted-foreground uppercase tracking-[0.2em] flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full bg-amber-500 animate-pulse" />
                  Interactive Chessboard
                </div>
                {(hoverNodeId || selectedNodeId || (isPlaying && currentNodeId)) && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-amber-500/10 text-amber-300 px-2 py-0.5 text-[10px] uppercase tracking-[0.15em] animate-pulse font-mono">
                    Previewing State
                  </span>
                )}
              </div>
              ...
              <div className="flex-1 flex items-center justify-center min-h-0">
                <div className="relative aspect-square w-full max-w-[340px] max-h-[340px] rounded-2xl overflow-hidden shadow-2xl shadow-black/80 border border-white/5 bg-slate-900 ring-1 ring-white/10">
            ...
            {/* ---------------------------------------------------- */}
            {/* ROW 1 RIGHT: SEARCH TREE CARD */}
            {/* ---------------------------------------------------- */}
            <div className="glass-strong rounded-3xl border border-white/10 relative overflow-hidden flex flex-col h-[500px] min-w-0">
              {/* Integrated Header Toolbar */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between px-6 py-4 border-b border-white/5 gap-3 bg-white/[0.01] shrink-0">
                <div className="flex items-center gap-2">
                  <Search className="h-4 w-4 text-amber-500" />
                  <span className="text-[11px] font-mono font-semibold text-muted-foreground uppercase tracking-[0.15em]">
                    Engine Thinking Tree
                  </span>
                </div>
                <div className="flex items-center justify-between sm:justify-end gap-3 w-full sm:w-auto">
                  <div className="text-[9px] font-mono text-muted-foreground uppercase tracking-wider flex items-center gap-1">
                  ...
                  <div className="flex gap-1">
                    <button
                      ...
                      className="h-6 w-6 glass hover:bg-white/10 rounded-lg flex items-center justify-center text-muted-foreground hover:text-white cursor-pointer"
                      title="Zoom In"
                    >
                    ...
                    <button
                      ...
                      className="px-2 h-6 glass hover:bg-white/10 rounded-lg text-[9px] font-mono text-muted-foreground hover:text-white flex items-center justify-center cursor-pointer"
                    >
                      Reset
                    </button>
                  </div>
              ...
              <div
                className="w-full flex-1 overflow-hidden cursor-grab active:cursor-grabbing relative select-none min-h-0"
              ...
              {hoveredNode && hoveredNode.pruned && (
                <div className="absolute bottom-4 left-6 right-6 bg-rose-950/90 border border-rose-800/30 rounded-xl p-3 text-xs text-rose-200 flex items-start gap-2 backdrop-blur-md shadow-xl animate-fade-in z-20">
            ...
            {/* ---------------------------------------------------- */}
            {/* ROW 2 LEFT: CONTROLLER CARD */}
            {/* ---------------------------------------------------- */}
            <div className="glass-strong rounded-3xl p-6 border border-white/10 h-[280px] flex flex-col justify-between">
              <div className="text-[11px] font-mono text-muted-foreground uppercase tracking-[0.2em] flex items-center gap-1.5 shrink-0">
              ...
              {/* Toggles */}
              <div className="grid grid-cols-2 gap-4 py-2 border-y border-white/5 my-auto">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    disabled={isPlaying}
                    className={cn(
                      "w-8 h-4 rounded-full p-0.5 transition-colors focus:outline-none shrink-0 cursor-pointer",
                      enableAlphaBeta ? "bg-amber-500" : "bg-white/10"
                    )}
                  >
                    <div
                      className={cn(
                        "w-3 h-3 rounded-full bg-slate-900 transition-transform duration-300",
                        enableAlphaBeta ? "translate-x-4" : "translate-x-0"
                      )}
                    />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs text-muted-foreground flex items-center gap-1.5 select-none">
                    <Maximize2 className="h-3.5 w-3.5 text-yellow-400 shrink-0" />
                    Show PV
                  </span>
                  <button
                  ...
                  >
                    <div
                      className={cn(
                        "w-3 h-3 rounded-full bg-slate-900 transition-transform duration-300",
                        highlightPV ? "translate-x-4" : "translate-x-0"
                      )}
                    />
                  </button>
                </div>
              ...
              {/* Playback & Thinking Controls */}
              <div className="space-y-2 shrink-0">
                <div className="flex gap-2">
                  <button
                    onClick={handleStartThinking}
                    disabled={isPlaying}
                    className="flex-1 py-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 disabled:from-white/5 disabled:to-white/5 disabled:text-muted-foreground text-slate-950 font-semibold text-xs rounded-xl shadow-lg shadow-amber-950/10 active:scale-[0.98] transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                  ...
                  <button
                    onClick={handleReset}
                    className="px-3 bg-white/5 border border-white/5 hover:bg-white/10 text-white rounded-xl active:scale-[0.98] transition-all flex items-center justify-center cursor-pointer"
                    title="Reset Board"
                  >
                    <RotateCcw className="h-3.5 w-3.5 text-muted-foreground hover:text-white" />
                  </button>
                </div>

                {isPlaying && (
                  <div className="flex items-center justify-between text-[9px] font-mono text-amber-400 animate-pulse">
                    <span>Searching...</span>
                    <span>{playbackIdx + 1}/{searchResult.events.length}</span>
                  </div>
                )}
            ...
            <div className="glass-strong rounded-3xl p-6 border border-white/10 h-[280px] flex flex-col justify-between">
              ...
              <div className="grid grid-cols-2 gap-4 my-auto">
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-3 flex flex-col justify-between">
                  <span className="text-[9px] text-muted-foreground font-mono uppercase tracking-wider block">Best Move</span>
                  <span className="text-lg font-bold text-emerald-400 font-mono mt-0.5 block">
                    {searchResult.bestMove?.san || "-"}
                  </span>
                </div>
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-3 flex flex-col justify-between">
                  <span className="text-[9px] text-muted-foreground font-mono uppercase tracking-wider block">Evaluation</span>
                  <span className={cn(
                    "text-lg font-bold font-mono mt-0.5 block",
                    searchResult.score > 0 ? "text-emerald-400" : searchResult.score < 0 ? "text-rose-400" : "text-white"
                  )}>
              ...
              <div className="grid grid-cols-4 gap-2 pt-3 border-t border-white/5 shrink-0">
                <div className="flex flex-col">
                  <span className="text-[8px] text-muted-foreground font-mono uppercase tracking-wider">Nodes</span>
                  <span className="text-xs font-semibold text-white font-mono mt-0.5">
                    {isPlaying ? (playbackIdx * 35).toLocaleString() : searchResult.telemetry.nodesSearched.toLocaleString()}
                  </span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[8px] text-muted-foreground font-mono uppercase tracking-wider">Speed</span>
                  <span className="text-xs font-semibold text-white font-mono mt-0.5">
                    {estimatedNps.toLocaleString()}
                  </span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[8px] text-muted-foreground font-mono uppercase tracking-wider">Savings</span>
                  <span className="text-xs font-semibold text-blue-400 font-mono mt-0.5">
                    {searchResult.telemetry.savingsPercent}%
                  </span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[8px] text-muted-foreground font-mono uppercase tracking-wider">Branch</span>
                  <span className="text-xs font-semibold text-purple-400 font-mono mt-0.5">
            ...
            {/* ---------------------------------------------------- */}
            {/* ROW 3 LEFT: HOW IT WORKS CARD */}
            {/* ---------------------------------------------------- */}
            <div className="glass-strong rounded-3xl p-6 border border-white/10 h-[280px] flex flex-col">
              <div className="text-[11px] font-mono text-muted-foreground uppercase tracking-[0.2em] flex items-center gap-1.5 shrink-0 mb-3">
                <BookOpen className="h-3.5 w-3.5 text-amber-500" />
                How It Works
              </div>

              {/* Explanations List with scroll containment */}
              <div className="flex-1 overflow-y-auto space-y-4 pr-1 min-h-0 scrollbar-thin">
                <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-3.5">
                  ...
                  <p className="text-muted-foreground text-[11px] leading-relaxed mt-1">
                    White searches for the move that yields the highest score, assuming Black will reply with the move that minimizes the score. By searching ahead and propagating leaf evaluations back up, the engine decides the mathematically optimal move.
                  </p>
                </div>

                <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-3.5">
                  <h4 className="font-semibold text-xs text-blue-300 flex items-center gap-1.5">
                    Alpha-Beta Pruning
                  </h4>
                  <p className="text-muted-foreground text-[11px] leading-relaxed mt-1">
                    If the engine discovers a move is worse than an alternative it has already calculated, it stops evaluating that branch immediately. It is mathematically proven to yield the same result but up to 10x faster.
                  </p>
                </div>

                <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-3.5">
                  <h4 className="font-semibold text-xs text-emerald-300 flex items-center gap-1.5">
                    Evaluation Function
                  </h4>
                  <p className="text-muted-foreground text-[11px] leading-relaxed mt-1">
                    Scores leaf positions by counting material (+1.0 pawn, +3.2 knight) and scoring placement from center control tables (Piece-Square Tables PST) to quantify relative positional advantages.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="glass-strong rounded-3xl p-6 border border-white/10 h-[280px] flex flex-col justify-between">
              ...
              {/* Preset Select & Turn display */}
              <div className="grid grid-cols-2 gap-4 my-auto">
                <div className="flex flex-col gap-1.5">
                  <span className="text-[10px] text-muted-foreground font-mono uppercase tracking-wider block">Position Preset</span>
                  <select
                    value={selectedPreset}
                    onChange={(e) => handlePresetChange(e.target.value)}
                    disabled={isPlaying}
                    className="glass w-full px-2.5 py-1.5 rounded-xl text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-amber-500 cursor-pointer"
                  >
                    {PRESETS.map((p) => (
                ...
                <div className="flex flex-col gap-1.5">
                  <span className="text-[10px] text-muted-foreground font-mono uppercase tracking-wider block">Active Turn</span>
                  <div className="flex items-center gap-2 bg-white/[0.02] border border-white/5 rounded-xl px-3 py-1.5 h-[34px]">
                    <span className={cn(
                      "h-3 w-3 rounded-full border border-white/20",
                      boardState.turn === "w" ? "bg-white" : "bg-slate-950"
                    )} />
                    <span className="text-xs font-semibold text-white">
                      {boardState.turn === "w" ? "White to move" : "Black to move"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-1.5 shrink-0">
                <span className="text-[10px] text-muted-foreground font-mono uppercase tracking-wider block">Active FEN String</span>
                <div 
                  className="bg-slate-950/40 border border-white/5 rounded-xl px-3 py-2 font-mono text-[9px] text-muted-foreground truncate select-all cursor-pointer flex justify-between items-center"
                  title="Click to select all FEN text"
                  onClick={(e) => {
                ...
                >
                  <span>{stateToFen(boardState)}</span>
                </div>
              </div>
            </div>
          </motion.div>
